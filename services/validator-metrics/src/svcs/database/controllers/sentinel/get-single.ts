...
export async function getSentinelValidatorStats(
  ...
  historyOpts?:{
    ...
    return db().transaction(async (dbTx)=> {
      const validator = await dbTx.select({
        validator: {
        ...
        blocks: { total: SentinelBlockTable.total, missed: SentinelBlockTable.missed, lastSeenAt: SentinelBlockTable.lastSeenAt, lastSeenAtSlot:SentinelBlockTable.lastSeenAtSlot },
        attestations: { total: SentinelAttestationTable.total, missed: SentinelAttestationTable.missed, lastSeenAt: SentinelAttestationTable.lastSeenAt, lastSeenAtSlot:SentinelAttestationTable.lastSeenAtSlot },
      })
      ...
      if(historyOpts){
       historyArray = await dbTx.select()
      ...
      return sentinelValidatorStatsSchema.parse({...validator.validator, blocks: validator.blocks, attestations:validator.attestations, history: historyArray})
    })
}