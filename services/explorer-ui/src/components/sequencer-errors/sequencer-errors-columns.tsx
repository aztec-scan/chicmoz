...
export const SequencerErrorsTableColumns: ColumnDef<SequencerErrorTableSchema>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="font-mono text-xs max-w-xs truncate" title={String(displayName)}>
          {String(displayName)}
        </div>
      );
    },
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const errorData = {
        ...
        createdAt: row.original.createdAt instanceof Date ? row.original.createdAt.toISOString() : row.original.createdAt,
        lastSeenAt: row.original.lastSeenAt instanceof Date ? row.original.lastSeenAt.toISOString() : row.original.lastSeenAt,
