...
import { DroppedTxsEvent, PendingTxsEvent } from "@chicmoz-pkg/message-registry";
...
export const onPendingTxs = async (pendingTxs: Tx[]) => {
  try {
    ...
    const currentPendingTxs: PendingTx[] = await Promise.all(
    ...
    const newTxs = currentPendingTxs.filter(
      (currentTx) => !storedPendingTxs.some((storedTx) => storedTx.txHash === currentTx.txHash)
    );

    // Find dropped txs (in stored but not in current)
    const droppedTxs = storedPendingTxs.filter(
      (storedTx) => !currentPendingTxs.some((currentTx) => currentTx.txHash === storedTx.txHash)
    );
    ...
    if (newTxs.length > 0) {
      ...
      logger.info(`📤 Published PENDING_TXS_EVENT for ${newTxs.length} new txs`);
    }
    ...
    if (droppedTxs.length > 0) {
      ...
      logger.info(`🗑️ Published DROPPED_TXS_EVENT for ${droppedTxs.length} dropped txs`);
    }
  } catch (error) {
